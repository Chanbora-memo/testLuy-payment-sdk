...
class Logger {
  ...
  _parseLevel(level) {
    ...
    return LOG_LEVEL_MAP[levelStr] !== undefined
      ? LOG_LEVEL_MAP[levelStr]
      : LogLevel.WARN;
  }
  ...
  debug(...args) {
    if (this.options.level <= LogLevel.DEBUG) {
      const maskedArgs = this.options.maskSensitive
        ? args.map((arg) => this._maskSensitiveData(arg))
        : args;
    ...
    if (this.options.level <= LogLevel.ERROR) {
      ...
      if (error.config) {
        ...
        if (options.includeHeaders && error.config.headers) {
          logData.request.headers = this._maskSensitiveData(
      ...
      if (error.response) {
        ...
        if (options.includeHeaders && error.response.headers) {
          logData.response.headers = this._maskSensitiveData(
    ...
    if (typeof process !== "undefined" && process.env) {
      ...
      if (process.env.LOG_MASK_SENSITIVE) {
        options.maskSensitive =
          process.env.LOG_MASK_SENSITIVE.toLowerCase() === "true";
      }
      ...
      if (process.env.LOG_ENABLE_METRICS) {
        options.enableMetrics =
          process.env.LOG_ENABLE_METRICS.toLowerCase() === "true";
      }
    }
  ...
  endRequestTracking(trackingInfo, result = {}) {
    ...
    if (this.metrics.performance.requestTimes.length > 100) {
      this.metrics.performance.requestTimes =
        this.metrics.performance.requestTimes.slice(-100);
    }
    ...
    if (result.success) {
      ...
      if (result.statusCode) {
        const count =
          this.metrics.errors.byStatusCode.get(result.statusCode) || 0;
    ...
    return {
      ...
      troubleshooting: {
        commonIssues: Object.fromEntries(
          this.metrics.troubleshooting.commonIssues
        ),
  ...
  logMetricsSummary() {
    ...
    this.info("Performance Metrics Summary:", {
      ...
      averageResponseTime: `${metrics.performance.averageResponseTime.toFixed(
        2
      )}ms`,
      minResponseTime: `${
  ...
  logTroubleshootingSuggestions() {
    ...
    suggestions.forEach((suggestion) => {
      ...
      this.info(
        `${priorityLabel} ${actionableLabel} ${suggestion.issue}: ${suggestion.suggestion}`
      );
    });
  }
      ...
      metrics: {
        ...
        performance: {
          ...
          minResponseTime:
            metrics.performance.minResponseTime === Infinity
              ? 0
              : metrics.performance.minResponseTimeTime === Infinity
              ? 0
              : metrics.performance.minResponseTime,
  ...
  logDiagnosticReport() {
    ...
    this.info(
      `- Cloudflare Blocked Requests: ${report.summary.cloudflareBlockedRequests}`
    );
    ...
    if (report.recommendations.length > 0) {
      ...
      report.recommendations.forEach((suggestion) => {
        const priorityLabel = suggestion.priority.toUpperCase();
        const actionableLabel = suggestion.actionable
          ? "[ACTIONABLE]"
          : "[INFO]";
        this.info(
          `- ${priorityLabel} ${actionableLabel} ${suggestion.issue}: ${suggestion.suggestion}`
  ...
  _generateTroubleshootingSuggestions(issueType, details) {
    ...
    switch (issueType) {
      case "rate_limit":
        suggestions.push({
          issue: "Rate Limiting",
          suggestion:
            "Consider upgrading your subscription plan for higher rate limits",
          priority: "high",
          actionable: true,
        });
        suggestions.push({
      ...
      case "network_timeout":
        ...
        suggestions.push({
          issue: "Network Timeouts",
          suggestion:
            "Consider implementing circuit breaker pattern for unstable networks",
          priority: "low",
          actionable: true,
        });
        break;
      ...
      case "server_error":
        suggestions.push({
          issue: "Server Errors",
          suggestion:
            "Wait and retry later as the API service might be experiencing issues",
          priority: "high",
          actionable: false,
        });
        suggestions.push({
      ...
      case "client_error":
        suggestions.push({
          issue: "Client Errors",
          suggestion:
            "Verify your request parameters match the API documentation",
          priority: "high",
          actionable: true,
        });
        suggestions.push({
          issue: "Client Errors",
          suggestion:
            "Check for any recent API changes that might affect your integration",
          priority: "medium",
          actionable: true,
        });
        break;
    ...
    for (const suggestion of suggestions) {
      const exists = this.metrics.troubleshooting.suggestions.some(
        (s) =>
          s.issue === suggestion.issue && s.suggestion === suggestion.suggestion
      );
    ...
    if (this.metrics.troubleshooting.suggestions.length > 20) {
      this.metrics.troubleshooting.suggestions =
        this.metrics.troubleshooting.suggestions.slice(-20);
    }
  }
  ...
  logDiagnosticReport() {
    ...
    if (report.recommendations.length > 0) {
      ...
      report.recommendations.forEach((rec, index) => {
        this.info(
          `${index + 1}. [${rec.priority.toUpperCase()}] ${rec.suggestion}`
        );
      });
    }
